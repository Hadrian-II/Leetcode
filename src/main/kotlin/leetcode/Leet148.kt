...
fun sortList(head: ListNode?): ListNode? {
    ...
    return mergeSort(head, head.`val`).first
}

fun mergeSort(start: ListNode, pivot: Int): Pair<ListNode, ListNode> {
    val before = ListNode(Integer.MAX_VALUE)
    var bAttach = before
    var beforeCount = 0
    var beforeSum = 0

    val middle = ListNode(Integer.MAX_VALUE)
    var mAttach = middle

    val after = ListNode(Integer.MAX_VALUE)
    var aAttach = after
    var afterCount = 0
    var afterSum = 0
    ...
    while (current != null) {
        ...
        if (current.`val` > pivot) {
            aAttach.next = current
            aAttach = current
            afterCount++
            afterSum += current.`val`
        } else if (current.`val` == pivot) {
            mAttach.next = current
            mAttach = current
        } else {
            bAttach.next = current
            bAttach = current
            beforeCount++
            beforeSum += current.`val`
        }

        current = next
    }

    val factory = ListNode(Int.MAX_VALUE)
    var fAppend = factory

    if (beforeCount == 1) {
        fAppend.next = before.next
        fAppend = bAttach
    } else if (beforeCount > 1) {
        val sorted = mergeSort(before.next!!, beforeSum / beforeCount)
        fAppend.next = sorted.first
        fAppend = sorted.second
    }

    if (middle.next != null) {
        fAppend.next = middle.next
        fAppend = mAttach
    }

    if (afterCount == 1) {
        fAppend.next = after.next
        fAppend = aAttach
    } else if (afterCount > 1) {
        val sorted = mergeSort(after.next!!, afterSum / afterCount)
        fAppend.next = sorted.first
        fAppend = sorted.second
    }

    return Pair(factory.next!!, fAppend)
}